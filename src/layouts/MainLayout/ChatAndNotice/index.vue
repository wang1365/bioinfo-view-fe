...
const websocketOnopen = () => {
    console.log('纳昂达可视化解读: WebSocket连接成功!')
}
const websocketOnerror = () => {
    console.log('纳昂达可视化解读: WebSocket连接发生错误!')
    reconnect()
}
...
const websocketOnmessage = (e) => {
    ...
    // console.log('纳昂达可视化解读: 收到新消息', e.data)
...
const websocketOnclose = (e) => {
    if (e && e.code) {
        console.log('纳昂达可视化解读: WebSocket连接已关闭:', e, e.code)
        reconnect()
    } else {
        console.log('纳昂达可视化解读: WebSocket连接已关闭。')
...
const reconnect = () => {
    ...
    timer.value = setTimeout(() => {
        console.log('纳昂达可视化解读: WebSocket尝试重连...')
        initWebSocket()
