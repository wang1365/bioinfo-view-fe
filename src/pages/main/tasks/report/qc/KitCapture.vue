<template>
    <div class="q-py-sm">
        <div class="text-bold">
            <span>{{
                    (onTarget.isTumorSample ? $t('OnTargetRateOfTumorSamples') : $t('OnTargetRateOfSamples')) + `：${onTarget.tumorTargetRate}`
                }}</span>
            <span
...
const columns = computed(() => {
    let result = [
        ...
        {
            title: onTarget.value.isTumorSample ? t('TumorValue') : t('Value'),
            dataIndex: 'k2',
            align: 'center',
            width: 120,
            customCell: groupCustomCell
        },
