<template>
    <q-page padding style="overflow-x: hidden; padding-top: 10px">
        <div class="row items-center q-py-sm">
            <h6>{{ $t('Rp2ConfigReport') }}</h6>
            <q-space />
            <q-btn :label="$t('Back')" icon="arrow_back" color="primary" flat @click="goBack()" />
        </div>
        <q-stepper v-if="viewConfigLoaded" v-model="step" ref="stepper" header-nav color="primary" animated>
            ...
            <q-step name="create" :title="$t('ReportDefineEnsureTab')" icon="receipt_long">
                <div>
                    ...
                    <div v-if="isStepDone('mutation')">
                        <div>
                            ...
                            <q-chip color="primary" text-color="white" v-if="viewConfig.mutation.showMutGermline">
                                ...
                                <span v-if="!stepData.mutation?.germline.selected && stepData.mutation?.germline.filtered">{{ $t('ReportDefineSearched') }}</span>
                                <span v-if="!stepData.mutation?.germline.selected && !stepData.mutation?.germline.filtered">{{ $t('ReportDefineNoData') }}</span>
                            </q-chip>
                            <q-chip color="primary" text-color="white" v-if="viewConfig.mutation.showMutSomatic">
                                ...
                                <span v-if="!stepData.mutation?.somatic.selected && stepData.mutation?.somatic.filtered">{{ $t('ReportDefineSearched') }}</span>
                                <span v-if="!stepData.mutation?.somatic.selected && !stepData.mutation?.somatic.filtered">{{ $t('ReportDefineNoData') }}</span>
                            </q-chip>
                            <q-chip color="primary" text-color="white" v-if="viewConfig.mutation.showMutWES">
                                ...
                                <span v-if="!stepData.mutation?.wes.selected && stepData.mutation?.wes.filtered">{{ $t('ReportDefineSearched') }}</span>
                                <span v-if="!stepData.mutation?.wes.selected && !stepData.mutation?.wes.filtered">{{ $t('ReportDefineNoData') }}</span>
                            </q-chip>
                        </div>
                    </div>
                    <div v-if="isStepDone('fusion')">
                        <div>
                            ...
                            <q-chip color="primary" text-color="white" v-if="viewConfig.fusion.showFusionGermline && samples.length <= 1">
                                ...
                                <span v-if="!stepData.fusion?.single.qt.selected && stepData.fusion?.single.qt.filtered">{{ $t('ReportDefineSearched') }}</span>
                                <span v-if="!stepData.fusion?.single.qt.selected && !stepData.fusion?.single.qt.filtered">{{ $t('ReportDefineNoData') }}</span>
                            </q-chip>
                            <q-chip color="primary" text-color="white" v-if="viewConfig.fusion.showFusionGermline && samples.length > 1">
                            ...
                            <q-chip color="primary" text-color="white" v-if="viewConfig.fusion.showFusionGermline && samples.length > 1">
                                ...
                                <span v-if="!stepData.fusion?.single.qn.selected && stepData.fusion?.single.qn.filtered">{{ $t('ReportDefineSearched') }}</span>
                                <span v-if="!stepData.fusion?.single.qn.selected && !stepData.fusion?.single.qn.filtered">{{ $t('ReportDefineNoData') }}</span>
                            </q-chip>
                            <q-chip color="primary" text-color="white" v-if="viewConfig.fusion.showFusionSomatic">
                                ...
                                <span v-if="!stepData.fusion?.normal?.selected && stepData.fusion?.normal?.filtered">{{ $t('ReportDefineSearched') }}</span>
                                <span v-if="!stepData.fusion?.normal?.selected && !stepData.fusion?.normal?.filtered">{{ $t('ReportDefineNoData') }}</span>
                            </q-chip>
                        </div>
                    </div>
                    <div v-if="isStepDone('copy_number_variation_wes')">
                        ...
                        <q-chip color="primary" text-color="white">
                            <span v-if="stepData.copy_number_variation_wes?.selected">{{ $t('ReportDefineSelected') }}</span>
                            <span v-if="!stepData.copy_number_variation_wes?.selected && stepData.copy_number_variation_wes?.filtered">{{ $t('ReportDefineSearched') }}</span>
                            <span v-if="!stepData.copy_number_variation_wes?.selected && !stepData.copy_number_variation_wes?.filtered">{{ $t('ReportDefineNoData') }}</span>
                        </q-chip>
                    </div>
                    <div v-if="isStepDone('copy_number_variation')">
                        <span class="text-bold">{{ $t('CopyNumberVariationAnalysis') }}</span>
                        <q-chip color="primary" text-color="white">
                            <span v-if="stepData.copy_number_variation?.table.selected">{{ $t('ReportDefineSelected') }}</span>
                            <span v-if="!stepData.copy_number_variation?.table.selected && stepData.copy_number_variation?.table.filtered">{{ $t('ReportDefineSearched') }}</span>
                            <span v-if="!stepData.copy_number_variation?.table.selected && !stepData.copy_number_variation?.table.filtered">{{ $t('ReportDefineNoData') }}</span>
                        </q-chip>
                    </div>
                ...
                <div class="text-center q-pa-md q-gutter-sm">
                    <q-btn :loading="saving" :label="$t('Save')" color="primary" @click="saveReport()" />
                    <q-btn :label="$t('Cancel')" color="red" @click="goBack()" />
                </div>
            </q-step>
        </q-stepper>
    </q-page>
...
import SelectedWesMutationTable from '../../defineReport/components/SelectedWesMutationTable.vue'
import SelectedWesCnvTable from '../../defineReport/components/SelectedWesCnvTable.vue'
import SelectedMutationTable from '../../defineReport/components/SelectedMutationTable.vue'
...
const loadExistingReport = () => {
    const url = `/report/report/by_sample?task_id=${taskId.value}&sample_id=${sampleId.value}`
    apiGet(
        url,
        (res) => {
            if (res.data && res.data.length > 0) {
                existingReport.value = res.data[0]
                reportComment.value = existingReport.value.comment || ''
                if (existingReport.value.query) {
                    try {
                        const parsedQuery = JSON.parse(existingReport.value.query)
                        stepData.value = parsedQuery
                        // Mark all steps as done based on loaded data
                        for (const key in parsedQuery) {
                            if (viewConfig.value[key]) {
                                viewConfig.value[key].stickDone = true
                            }
                        }
                    } catch (e) {
                        console.error('Failed to parse existing report query:', e)
                    }
                }
            }
        },
        {},
        (err) => {
            console.log('No existing report found or error fetching:', err)
        }
    )
}
...
const saveReport = () => {
    ...
    apiPost(
        ...
        {},
        (err) => {
            errorMessage(err.msg || t('SaveFailed'))
            saving.value = false
        }
    )
}
...
const loadIntros = () => {
    ...
    readTaskFile(taskId.value, `result_${suffix}.json`).then((res) => {
...
const loadViewConfig = () => {
    const suffix = langCode.value === 'en' ? 'EN' : 'CN'
    readTaskFile(taskId.value, `module_${suffix}.json`).then((res) => {
        let data = null
        const dict = {
            质控: 'qc',
            突变分析: 'mutation',
        ...
        // Normalize data to an object to avoid null/undefined issues during processing
        ...
        if (data) {
            ...
            if (Array.isArray(data.commonModules)) {
